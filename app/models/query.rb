...
# will return a Celluloid::Future, which on completion will yield an object of
# your choice.
class Query
  include Celluloid
  ...
  def self.queue(code, tag, reporter)
    QueryPool.future.run(code, tag, reporter)
  end

  ##
  # Runs a query.
  #
  # When a query proc terminates or times out, its return value (or associated
  # exception) and tag are sent to the reporter.  The reporter MUST respond to
  ...
  #                        to #success?
  # 3. #failure(ret, tag): called if the proc's return value responds falsy to
  #                        #success?
  # 4. #timeout(ret, tag): called if the proc timed out
  # 5. #error(ret, tag):   called if the proc raised StandardError or subclass
